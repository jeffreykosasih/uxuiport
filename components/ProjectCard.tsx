  ...
  return (
    <article className='group bg-white border border-highlight/30 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col h-full'>
      ...
      <div className='p-8 flex flex-col flex-grow'>
        <h3 className='text-2xl font-semibold text-text-primary mb-3 group-hover:text-hover transition-colors'>
          {title}
        </h3>

        <p className='text-text-primary/80 mb-6 leading-relaxed flex-grow font-light'>
          {description}
        </p>
        ...
        <div className='flex flex-wrap gap-2 mb-8'>
          {tags.map((tag) => (
            <span
              key={tag}
              className='px-3 py-1 bg-primary border border-highlight text-text-primary text-sm rounded-full font-light'
            >
              {tag}
            </span>
          ))}
        ...
        <a
          href={link}
          className='inline-flex items-center gap-2 text-accent-dark font-medium hover:text-hover transition-colors group/link'
        >
          View Project
